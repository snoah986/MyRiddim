...
export function applyKaraokeOffset(rawLines, offsetSeconds, needsSync = false) {
  ...
  return [
    { start: 0, end: offset, text: '♪ [Cinematic Intro] ♪', words: [], isIntro: true },
    ...lines.map(line => ({
      ...line,
...
export function applyVideoOffset(rawLyrics, offsetSeconds, needsSync = false) {
  ...
  return [
    { time: 0, text: '♪ [Cinematic Intro] ♪', isIntro: true, duration: offset },
    ...lines.map(line => ({ ...line, time: Number((Number(line.time) + offset).toFixed(2)) }))
  ]
}
