<script>
  ...
  import { normalizePlayable, trackKey } from './lib/tracks.js'
  import { queue, hydrateQueue, persistQueue, seed, selectNext, selectPrevious, playUpcoming, playNext, addToQueue, appendTracks, removeUpcoming, clearUpcoming, reorderUpcoming, toggleShuffle, cycleRepeat } from './lib/queue.js'
  ...
  let quickPicks = [], recommendations = [], recommendationForId = null, recommendationLoading = false, recommendationRequest = 0, discoverTracks = [], discoverForId = null, discoverLoading = false, discoverRequest = 0, stats = { month: '', totalMinutes: 0, monthly: [], heavyRotation: [] }, quickScroller, searchQuery = '', searchResults = [], searchTimer, searchRequest = 0, searching = false, searchError = '', showCreate = false, newTitle = '', newDescription = '', preloadedTrackId = null
  ...
  async function loadRecommendations(track) {
    ...
    try {
      ...
      recommendations = (data.tracks || []).filter(item => item.videoId !== videoId)
    } catch (error) {
  ...
  async function loadDiscover(track) {
    ...
    try {
      ...
      discoverTracks = (data.recommendations || []).filter(item => item.videoId !== videoId)
    } catch {
  ...
  async function loadSmartMix() {
    ...
    try {
      ...
      smartMix = [...(data.mix || []).slice(0, 20), ...(data.discovery || []).slice(0, 10)]
  ...
  async function playTrack(track, index = 0, playlist = full) {
    ...
    if (gaplessHandoff) { loadingTrack = false; return }
    try {
  ...
  function searchChanged(valueOrEvent) {
    const value = typeof valueOrEvent === 'string' ? valueOrEvent : valueOrEvent?.currentTarget?.value ?? ''
    searchQuery = value
    ...
    searchTimer = setTimeout(async () => {
      try {
        const response = await apiFetch(`/api/search?q=${encodeURIComponent(value.trim())}`)
        const data = await response.json()
...
<main class:with-player={currentTrack} class:home-shell={!checking && authenticated && !artistData && !albumData && !full}>
  ...
  {:else if full}<div in:fade={{ duration: 200 }} out:fade={{ duration: 120 }}><SongPage playlist={full} track={currentTrack} onTrack={(track, index) => { if (activeQueue.nowPlaying?.videoId === track.videoId) togglePlay(); else { seed(full.tracks, index) } }} onPlay={() => full?.tracks?.[0] && playQueue(full.tracks)} onShuffle={() => playQueueShuffled(full?.tracks)} onPlayNext={playNextTrack} onAddToQueue={appendTrack} onAddToPlaylist={openAddModal} onStartMix={startMix} onRemoveTrack={removeTrackFromPlaylist} /></div>    {:else}<HomeView searchQuery={searchQuery} onSearchChanged={searchChanged} searchResults={searchResults} searching={searching} searchError={searchError} homeView={homeView} onViewChange={setHomeView} sessionState={sessionState} sessionBannerDismissed={sessionBannerDismissed} onReconnect={reauthenticate} onDismissSession={() => sessionBannerDismissed = true} libraryCached={libraryCached} onDismissOffline={() => libraryCached = false} playlists={playlists} playlistsError={playlistsError} playlistsLoaded={playlistsLoaded} onRetryPlaylists={loadPlaylists} onOpenPlaylist={open} onCreatePlaylist={() => showCreate = true} onOpenSmartCreator={() => showSmartModal = true} onOpenSettings={() => settingsOpen = true} smartPlaylists={smartPlaylists} smartPlaylistsLoading={smartPlaylistsLoading} smartPlaylistsError={smartPlaylistsError} onRefreshSmart={loadSmartPlaylists} onOpenSmartPlaylist={openSmartPlaylist} stats={stats} quickPicks={quickPicks} smartMix={smartMix} recommendations={recommendations} currentTrack={currentTrack} recommendationLoading={recommendationLoading} likedTracks={likedTracks} likedLoading={likedLoading} likedError={likedError} onRetryLiked={loadLiked} onPlayQueue={playQueue} onPlayNext={playNextTrack} onAddToQueue={appendTrack} onAddToPlaylist={openAddModal} onStartMix={startMix} onOpenBrowse={openBrowse} onArtworkLoad={extractAccent} moods={moods} activeMood={activeMood} moodPlaylists={moodPlaylists} moodLoading={moodLoading} onOpenMood={openMood} onOpenMoodPlaylist={(item) => item.browseId && open({id: item.browseId, title: item.title})} discoverTracks={discoverTracks} discoverLoading={discoverLoading} favoriteArtists={favoriteArtists} rotationTracks={rotationTracks} recentPlaylists={recentPlaylists} ytmShelves={ytmShelves} ytmFeedLoading={ytmFeedLoading} onRetryYtmFeed={loadYtmFeed} onOpenArtistEntity={openArtistEntity} onOpenAlbumEntity={openAlbumEntity} onOpenRecentPlaylist={(item) => item?.id && open({id: item.id, title: item.title})} />{#if false}<div in:fade={{ duration: 220 }} out:fade={{ duration: 140 }} class="view-transition"><div class="home-head" data-tauri-drag-region><div class="search-wrap"><input class="global-search" bind:value={searchQuery} on:input={searchChanged} placeholder="Search YouTube Music" aria-label="Search YouTube Music" /><kbd>Ctrl K</kbd></div><h1>My Music</h1>{#if sessionState === 'expired'}<button class="status expired" on:click={reauthenticate} title="Reconnect your YouTube Music session">⚠ Session expired — Reconnect</button>{/if}<button class="new-playlist" on:click={() => showCreate = true}>＋ New Playlist</button><button class="smart-playlist-launcher" on:click={() => showSmartModal = true} title="Create a local playlist from listening rules">⚡ Smart Playlist</button><button class="settings-toggle" on:click={() => settingsOpen = true} aria-label="Open settings" title="Settings">⚙</button><WindowControls /></div>{#if authError}<p class="error" role="alert">{authError}</p>{/if}
    ...
    <div class="chips" role="tablist" aria-label="Browse your library">
      ...
      <button class="chip" class:active={homeView === 'stats'} on:click={() => setHomeView('stats')}>Listening Stats</button>
    </div>
    {#if homeView === 'home' && (smartPlaylists.length || smartPlaylistsLoading || smartPlaylistsError)}
      <section class="smart-collections" aria-labelledby="smart-collections-title">
        <div class="section-head"><div><p class="eyebrow">LOCAL RECIPES</p><h2 id="smart-collections-title">Smart Collections</h2></div><button class="smart-refresh" type="button" on:click={loadSmartPlaylists} disabled={smartPlaylistsLoading}>{smartPlaylistsLoading ? 'Loading…' : 'Refresh'}</button></div>
        {#if smartPlaylistsError}<p class="empty-shelf">{smartPlaylistsError}</p>{:else if smartPlaylistsLoading && !smartPlaylists.length}<p class="empty-shelf">Loading saved collections…</p>{:else}<div class="smart-list">{#each smartPlaylists as playlist (playlist.id)}<button class="smart-collection" type="button" on:click={() => openSmartPlaylist(playlist)}><span class="smart-icon">⚡</span><span class="smart-copy"><strong>{clean(playlist.name)}</strong><small>{playlist.limit || 50} tracks · updates from listening history</small></span><span class="smart-live">Live&nbsp; →</span></button>{/each}</div>{/if}
      </section>
    {/if}
    {#if homeView === 'stats'}
      <StatsView onPlayTrack={(track) => playQueue([track])} />
    {/if}
    {#if homeView === 'home' || homeView === 'recent'}
    <section class="rotation"><div><h2>Heavy Rotation</h2><p class="rotation-copy">{stats.month ? stats.month + ' · ' : ''}{stats.totalMinutes} minutes listened</p></div>{#if stats.monthly[0]}<button class="recap" on:click={() => playQueue(stats.monthly)}><div class="recap-art">{#if stats.monthly[0].thumbnail}<img src={stats.monthly[0].thumbnail} referrerpolicy="no-referrer" alt="" />{:else}<span>♫</span>{/if}</div><div><strong>{clean(stats.monthly[0].title)}</strong><span>{clean(stats.monthly[0].artist)}</span></div><b>▶</b></button>{/if}{#if stats.heavyRotation[0]}<button class="recap" on:click={() => playQueue(stats.heavyRotation)}><div class="recap-art">{#if stats.heavyRotation[0].thumbnail}<img src={stats.heavyRotation[0].thumbnail} referrerpolicy="no-referrer" alt="" />{:else}<span>♫</span>{/if}</div><div><strong>On Repeat</strong><span>{clean(stats.heavyRotation[0].title)}</span></div><b>▶</b></button>{/if}</section>
    {/if}
    {#if homeView === 'recent' && stats.heavyRotation.length}<MediaShelf title="Your Heavy Rotation" tracks={stats.heavyRotation} onPlayTrack={(t, i) => playQueue(stats.heavyRotation, i)} onStartMix={startMix} />{/if}
    {#if homeView === 'home' || homeView === 'discover'}
    {#if quickPicks.length}<section class="quick"><div class="section-head"><h2>Quick Picks</h2><div class="carousel-controls"><button on:click={() => quickScroller?.scrollBy({left:-340, behavior:'smooth'})} aria-label="Scroll Quick Picks left">‹</button><button on:click={() => quickScroller?.scrollBy({left:340, behavior:'smooth'})} aria-label="Scroll Quick Picks right">›</button></div></div><div class="quick-row" bind:this={quickScroller}>{#each quickPicks as t, i (i)}<div class="quick-card-wrap mixable-track"><button class="quick-card" on:click={() => playQueue(quickPicks, i)}><div class="quick-art">{#if t.thumbnail}<img src={t.thumbnail} referrerpolicy="no-referrer" alt="" />{:else}<span>♫</span>{/if}<i>▶</i></div><strong>{clean(t.title)}</strong><span>{clean(t.artist)}</span></button><StartMixButton track={t} onStartMix={startMix} /></div>{/each}</div></section>{/if}
    {/if}
    {#if homeView !== 'stats' && smartMix.length}<MediaShelf tracks={smartMix} title="Personal Mix" onPlayTrack={(t, i) => playQueue(smartMix, i)} onStartMix={startMix} />{/if}
    {#if homeView !== 'stats' && currentTrack && (recommendationLoading || recommendations.length)}<section class="recommendations">{#if recommendationLoading}<div class="section-head"><h2>Finding something similar…</h2></div>{:else if recommendations.length}<MediaShelf tracks={recommendations} title={`Because you’re playing ${clean(currentTrack.title)}`} onPlayTrack={(t, i) => playQueue(recommendations, i)} onStartMix={startMix} />{:else}<div class="section-head"><h2>Similar songs</h2></div><p class="empty-shelf">No similar songs found yet.</p>{/if}</section>{/if}
    {#if homeView === 'discover' && moods.length}<section class="moods"><div class="section-head"><h2>Moods &amp; Genres</h2></div>{#each moods as section (section.title)}<div class="mood-group"><p class="mood-section-title">{section.title}</p><div class="chips">{#each section.categories as category (category.title)}<button class="chip" class:active={activeMood === category.title} on:click={() => openMood(category)}>{category.title}</button>{/each}</div></div>{/each}{#if moodLoading}<p class="empty-shelf">Loading playlists…</p>{:else if activeMood && moodPlaylists.length}<div class="card-row">{#each moodPlaylists as playlist, i (playlist.browseId || i)}<button class="browse-card" on:click={() => playlist.browseId && open({id: playlist.browseId, title: playlist.title})}><div class="card-art">{#if playlist.thumbnail}<img src={playlist.thumbnail} referrerpolicy="no-referrer" alt="" loading="lazy" />{:else}<span>♫</span>{/if}</div><strong>{clean(playlist.title)}</strong><span>{clean(playlist.count || 'Mood playlist')}</span></button>{/each}</div>{:else if activeMood}<p class="empty-shelf">No playlists for this mood yet.</p>{/if}</section>{/if}
    {#if homeView === 'favorites'}
    {#if likedTracks.length}<MediaShelf title="Favorites" tracks={likedTracks} onPlayTrack={(t, i) => playQueue(likedTracks, i)} onStartMix={startMix} />
    ...
    {:else if playlists.length}<div class="pl-grid">{#each playlists as p, i (p.id)}<button class="pl" class:hero={i === 0 && p.count > 0} class:ghost={p.count === 0} on:click={() => open(p)}><div class="pl-art">{#if p.thumbnail}<img src={p.thumbnail} referrerpolicy="no-referrer" alt="" loading="lazy" />{:else}<span>♫</span>{/if}<span class="float-play">▶</span></div><div class="pl-title">{clean(p.title)}</div><div class="pl-count">{p.count === 0 ? 'Empty' : (p.count + (p.count === 1 ? ' track' : ' tracks'))}</div></button>{/each}</div>
