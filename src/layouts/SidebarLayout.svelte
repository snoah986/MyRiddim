...
<div class="shell sidebar-shell">
  ...
  <div class="shell-main">
    <header class="shell-topbar" data-tauri-drag-region>
      <label class="shell-search"><span aria-hidden="true">⌕</span><input class="global-search" value={searchQuery} on:input={onSearchChanged} placeholder="Search your library" aria-label="Search your library" /><kbd>Ctrl K</kbd></label>
      <div class="shell-actions"><button on:click={onCreatePlaylist}>+ Playlist</button><button on:click={onOpenSmartCreator}>Smart</button><button class="settings" on:click={onOpenSettings} aria-label="Open settings">⚙</button><WindowControls /></div>
    </header>
...
<style>
  .shell { min-height:100vh; color:#f4f4f5; background:#09090b; font-family:Inter,ui-sans-serif,system-ui,sans-serif; }
  .sidebar-shell { display:grid; grid-template-columns:240px minmax(0,1fr); }
  .rail { position:sticky; top:0; display:flex; height:100vh; box-sizing:border-box; flex-direction:column; padding:24px 16px 18px; border-right:1px solid #ffffff0f; background:#0b0b0d; }
