<script>
  ...
  import { normalizeArtists, normalizeAlbum, openArtist, openAlbum } from '../utils/navigation.js'

  export let onOpenArtistEntity = () => {}
  export let onOpenAlbumEntity = () => {}
  export let onOpenRecentPlaylist = () => {}
  function openArtistEntityFromTrack(artist, trackId) {
    const resolved = artist?.id ?? trackId ?? artist?.name ?? null
    if (resolved) normalizeArtists({ artists: artist })?.forEach(item => onOpenArtistEntity(item.id ?? item.name, item.name))
  }
  function openAlbumEntityFromTrack(album, track) {
    onOpenAlbumEntity(album?.id ?? null, album?.name ?? track?.album ?? null, album?.artist ?? track?.artist ?? null)
  }
