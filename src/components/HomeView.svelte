  ...
  {:else if homeView === 'home' || homeView === 'recent' || homeView === 'discover'}
    ...
    {#if homeView === 'home' || homeView === 'discover'}
      ...
      {#if quickPicks.length}<section class="section collage-section"><div class="section-head"><div><h2 class="display">Quick picks</h2><p>Fresh from your listening habits.</p></div><span>{quickPicks.length} tracks</span></div><div class="collage">
        <div class="tile-wrap tile-wrap-lead mixable-track"><button class="tile lead" style={cardStyle(quickPicks[0])} on:click={() => play(quickPicks, 0)} on:contextmenu={(event) => openContextMenu(event, quickPicks[0])}>{#if quickPicks[0]?.thumbnail}<img class="tile-image" src={quickPicks[0].thumbnail} on:load={(event) => artLoaded(event, quickPicks[0])} referrerpolicy="no-referrer" alt="" />{/if}<span class="tile-scrim"></span><span class="tile-caption"><strong class="display">Quick picks</strong><small>Built from your recent listening</small></span></button><StartMixButton track={quickPicks[0]} onStartMix={onStartMix} /></div>
        {#each quickPicks.slice(1, 3) as item, index}<div class="tile-wrap tile-wrap-medium mixable-track"><button class="tile medium" style={cardStyle(item)} on:click={() => play(quickPicks, index + 1)} on:contextmenu={(event) => openContextMenu(event, item)}>{#if item.thumbnail}<img class="tile-image" src={item.thumbnail} on:load={(event) => artLoaded(event, item)} referrerpolicy="no-referrer" alt="" />{/if}<span class="tile-scrim"></span><strong class="tile-title">{clean(item.title)}</strong><small class="tile-artist">{clean(item.artist)}</small></button><StartMixButton track={item} onStartMix={onStartMix} /></div>{/each}
        {#each quickPicks.slice(3, 5) as item}<div class="tile-wrap tile-wrap-small mixable-track"><button class="tile small" style={cardStyle(item)} on:click={() => play([item])} on:contextmenu={(event) => openContextMenu(event, item)}><span class="small-art">{#if item.thumbnail}<img src={item.thumbnail} on:load={(event) => artLoaded(event, item)} referrerpolicy="no-referrer" alt="" />{:else}♫{/if}</span><span class="small-meta"><strong>{clean(item.title)}</strong><small class="entity-line"><span class="entity-link" on:click={(event) => openArtist(event, item)}>{clean(item.artist)}</span></small></span></button><StartMixButton track={item} onStartMix={onStartMix} /></div>{/each}
      </div></section>{/if}
...
<style>
  ...
  @media (max-width:800px) { .home-page { padding:20px 16px 140px; }.topbar { flex-wrap:wrap; }.wordmark { order:-1; width:100%; }.search { flex:1; width:auto; }.actions { margin-left:auto; }.btn { padding:9px 13px; }.collage { grid-template-columns:repeat(2,1fr); grid-auto-rows:110px; }.tile-wrap-lead { grid-column:span 2; grid-row:span 2; }.tile-wrap-medium { grid-column:span 1; grid-row:span 2; }.tile-wrap-small { grid-column:span 1; grid-row:span 1; }.row,.playlist-row { grid-template-columns:repeat(2,1fr); }.rowcard:nth-child(2),.playlist-card:nth-child(2) { margin-top:18px; }.rowcard:nth-child(4),.playlist-card:nth-child(4) { margin-top:28px; }.playlist-feature { grid-column:span 2; }.smart-list { grid-template-columns:1fr; }.hero { min-height:260px; gap:14px; }.hero-content { padding:28px 0 28px 26px; }.hero-title { max-width:100%; font-size:clamp(1.55rem,6vw,2.5rem); }.hero-art { width:138px; height:138px; margin-right:22px; border-radius:18px; } }
  @media (max-width:520px) { .actions .ghost,.expired { display:none; }.settings { width:32px; }.pills { margin-bottom:24px; }.hero { min-height:240px; align-items:flex-end; gap:8px; }.hero-content { padding:26px 0 25px 20px; }.hero-context { font-size:.57rem; letter-spacing:.12em; }.hero-title { font-size:1.55rem; }.hero-sub { max-width:calc(100vw - 190px); font-size:.78rem; }.hero-album { display:none; }.hero-actions { margin-top:16px; }.hero-play { height:36px; padding:0 15px; font-size:.7rem; }.hero-art { width:112px; height:112px; margin:0 16px 20px 0; border-radius:15px; } .collage { gap:9px; grid-auto-rows:95px; }.row,.playlist-row { gap:9px; }.rowcard,.playlist-card { padding:10px; border-radius:18px; }.rowcard:nth-child(2),.playlist-card:nth-child(2) { margin-top:12px; }.rowcard:nth-child(4),.playlist-card:nth-child(4) { margin-top:20px; } }
  @media (prefers-reduced-motion: reduce) { .ambient-glow,.hero-live-dot { animation:none; }.tile,.rowcard,.playlist-card,.btn,.hero-play { transition:none; } }
</style>
