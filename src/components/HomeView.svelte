...
<div class="home-page">
  ...
  <nav class="pills" aria-label="Browse your library">
  ...
  {:else if homeView === 'favorites'}
    <section class="section list-section"><div class="section-head"><div><h2 class="display">Favorites</h2><p>Your songs, kept close.</p></div></div>{#if likedLoading}<p class="empty">Loading favorites…</p>{:else if likedError}<div class="empty"><strong>Could not load favorites</strong><span>{likedError}</span><button class="pill-button" on:click={onRetryLiked}>Retry</button></div>{:else if likedTracks.length}<div class="row">{#each likedTracks.slice(0, 12) as item, index (item.videoId || index)}<article class="rowcard" style={cardStyle(item)}><button class="row-main" on:click={() => play(likedTracks, index)} on:contextmenu={(event) => openContextMenu(event, item)}><span class="art">{#if item.thumbnail}<img src={item.thumbnail} on:load={(event) => artLoaded(event, item)} referrerpolicy="no-referrer" alt="" />{:else}♫{/if}</span><strong>{clean(item.title)}</strong><small class="entity-line"><span class="entity-link" on:click={(event) => openArtist(event, item)}>{clean(item.artist)}</span></small></button><TrackContextMenu track={item} onPlayNext={onPlayNext} onAddToQueue={onAddToQueue} onAddToPlaylist={onAddToPlaylist} onStartMix={onStartMix} /></article>{/each}</div>{:else}<p class="empty">No favorites yet.</p>{/if}</section>
  {:else if homeView === 'home' || homeView === 'recent' || homeView === 'discover'}
    {#if heroTrack}<section class="hero mixable-track" style={`--hero-art: url("${heroArt}")`} aria-label="Now playing"><img class="hero-image" src={heroArt} on:load={(event) => artLoaded(event, heroTrack)} referrerpolicy="no-referrer" alt="" /><div class="hero-content"><p class="hero-context">{currentTrack ? 'Now playing' : 'From your library'}</p><h2 class="hero-title display">{clean(heroTitle)}</h2><p class="hero-sub">{clean(heroArtist)}</p>{#if heroTrack?.album}<p class="hero-album"><span class="entity-link" on:click={(event) => openAlbum(event, heroTrack)}>{clean(heroTrack.album)}</span></p>{/if}</div><button class="hero-play" on:click={() => play([heroTrack])} aria-label="Play {clean(heroTitle)}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 5v14l11-7z"></path></svg></button><StartMixButton track={heroTrack} onStartMix={onStartMix} /></section>{/if}
    ...
    {#if homeView === 'home' || homeView === 'discover'}
      ...
      {#if quickPicks.length}<section class="section collage-section"><div class="section-head"><div><h2 class="display">Quick picks</h2><p>Fresh from your listening habits.</p></div><span>{quickPicks.length} tracks</span></div><div class="collage">
        <div class="tile-wrap mixable-track"><button class="tile lead" style={cardStyle(quickPicks[0])} on:click={() => play(quickPicks, 0)} on:contextmenu={(event) => openContextMenu(event, quickPicks[0])}>{#if quickPicks[0]?.thumbnail}<img class="tile-image" src={quickPicks[0].thumbnail} on:load={(event) => artLoaded(event, quickPicks[0])} referrerpolicy="no-referrer" alt="" />{/if}<span class="tile-scrim"></span><span class="tile-caption"><strong class="display">Quick picks</strong><small>Built from your recent listening</small></span></button><StartMixButton track={quickPicks[0]} onStartMix={onStartMix} /></div>
        {#each quickPicks.slice(1, 3) as item, index}<div class="tile-wrap mixable-track"><button class="tile medium" style={cardStyle(item)} on:click={() => play(quickPicks, index + 1)} on:contextmenu={(event) => openContextMenu(event, item)}>{#if item.thumbnail}<img class="tile-image" src={item.thumbnail} on:load={(event) => artLoaded(event, item)} referrerpolicy="no-referrer" alt="" />{/if}<span class="tile-scrim"></span><strong class="tile-title">{clean(item.title)}</strong><small class="tile-artist">{clean(item.artist)}</small></button><StartMixButton track={item} onStartMix={onStartMix} /></div>{/each}
        {#each quickPicks.slice(3, 5) as item}<div class="tile-wrap mixable-track"><button class="tile small" style={cardStyle(item)} on:click={() => play([item])} on:contextmenu={(event) => openContextMenu(event, item)}><span class="small-art">{#if item.thumbnail}<img src={item.thumbnail} on:load={(event) => artLoaded(event, item)} referrerpolicy="no-referrer" alt="" />{:else}♫{/if}</span><span class="small-meta"><strong>{clean(item.title)}</strong><small class="entity-line"><span class="entity-link" on:click={(event) => openArtist(event, item)}>{clean(item.artist)}</span></small></span></button><StartMixButton track={item} onStartMix={onStartMix} /></div>{/each}
      </div></section>{/if}
    ...
    {#if homeView === 'home' || homeView === 'recent'}
      <section class="section"><div class="section-head"><div><h2 class="display">Heavy rotation</h2><p>The tracks you keep coming back to.</p></div><span>{heavy.length} tracks</span></div><div class="row">{#each heavy.slice(0, 4) as item, index (item.videoId || index)}<article class="rowcard" style={cardStyle(item)}><button class="row-main" on:click={() => play(heavy, index)} on:contextmenu={(event) => openContextMenu(event, item)}><span class="art">{#if item.thumbnail}<img src={item.thumbnail} on:load={(event) => artLoaded(event, item)} referrerpolicy="no-referrer" alt="" />{:else}♫{/if}</span><strong>{clean(item.title)}</strong><small class="entity-line"><span class="entity-link" on:click={(event) => openArtist(event, item)}>{clean(item.artist)}</span></small></button><TrackContextMenu track={item} onPlayNext={onPlayNext} onAddToQueue={onAddToQueue} onAddToPlaylist={onAddToPlaylist} onStartMix={onStartMix} /></article>{/each}</div>{#if !heavy.length}<p class="empty">Your listening history will shape this section.</p>{/if}</section>

      {#if rotationTracks.length}<section class="section"><div class="section-head"><div><h2 class="display">In rotation this month</h2><p>Your most-completed tracks over the last 30 days.</p></div><span>{rotationTracks.length} tracks</span></div><div class="row">{#each rotationTracks.slice(0, 4) as item, index (item.videoId || index)}<article class="rowcard" style={cardStyle(item)}><button class="row-main" on:click={() => play(rotationTracks, index)} on:contextmenu={(event) => openContextMenu(event, item)}><span class="art">{#if item.thumbnail}<img src={item.thumbnail} on:load={(event) => artLoaded(event, item)} referrerpolicy="no-referrer" alt="" />{:else}♫{/if}</span><strong>{clean(item.title)}</strong><small class="entity-line"><span class="entity-link" on:click={(event) => openArtist(event, item)}>{clean(item.artist)}</span></small></button><TrackContextMenu track={item} onPlayNext={onPlayNext} onAddToQueue={onAddToQueue} onAddToPlaylist={onAddToPlaylist} onStartMix={onStartMix} /></article>{/each}</div></section>{/if}
    {/if}

    {#if homeView !== 'stats' && (recommendationLoading || because.length)}<section class="section"><div class="section-head"><div><h2 class="display">Because you’re playing this</h2><p>{currentTrack ? clean(currentTrack.title) : 'Pick a track to get started.'}</p></div><span>{because.length} tracks</span></div>{#if recommendationLoading}<p class="empty">Finding a good follow-up…</p>{:else}<div class="row">{#each because.slice(0, 4) as item, index (item.videoId || index)}<article class="rowcard mixable-track" style={cardStyle(item)}><button class="row-main" on:click={() => play(because, index)} on:contextmenu={(event) => openContextMenu(event, item)}><span class="art">{#if item.thumbnail}<img src={item.thumbnail} on:load={(event) => artLoaded(event, item)} referrerpolicy="no-referrer" alt="" />{:else}♫{/if}</span><strong>{clean(item.title)}</strong><small class="entity-line"><span class="entity-link" on:click={(event) => openArtist(event, item)}>{clean(item.artist)}</span></small>{#if item.available_sources?.length}<span class="source-badges">{#each item.available_sources as source}<em>{source === 'soundcloud' ? 'SoundCloud' : 'YouTube'}</em>{/each}</span>{/if}</button><button class="add" on:click={() => onAddToQueue(item)} aria-label="Add {clean(item.title)} to queue">＋</button><StartMixButton track={item} onStartMix={onStartMix} /></article>{/each}</div>{/if}</section>{/if}

    {#if smartMix.length}<section class="section"><div class="section-head"><div><h2 class="display">Personal mix</h2><p>Built locally from what you actually play.</p></div><span>{smartMix.length} tracks</span></div><div class="row">{#each smartMix.slice(0, 4) as item, index (item.videoId || index)}<article class="rowcard mixable-track" style={cardStyle(item)}><button class="row-main" on:click={() => play(smartMix, index)} on:contextmenu={(event) => openContextMenu(event, item)}><span class="art">{#if item.thumbnail}<img src={item.thumbnail} on:load={(event) => artLoaded(event, item)} referrerpolicy="no-referrer" alt="" />{:else}♫{/if}</span><strong>{clean(item.title)}</strong><small class="entity-line"><span class="entity-link" on:click={(event) => openArtist(event, item)}>{clean(item.artist)}</span></small>{#if item.available_sources?.length}<span class="source-badges">{#each item.available_sources as source}<em>{source === 'soundcloud' ? 'SoundCloud' : 'YouTube'}</em>{/each}</span>{/if}</button><StartMixButton track={item} onStartMix={onStartMix} /></article>{/each}</div></section>{/if}
...
<style>
  ...
  .display { font-family:'Anton',Impact,sans-serif; font-weight:400; letter-spacing:.01em; }  .hero { position:relative; height:300px; margin-bottom:44px; overflow:hidden; border-radius:32px; background:#1c1512; }.hero-image,.tile-image { position:absolute; inset:0; width:100%; height:100%; object-fit:cover; }.hero::before { content:''; position:absolute; inset:0; z-index:1; background:linear-gradient(to top,#000b,transparent 62%); pointer-events:none; }  .hero-content { position:relative; z-index:2; display:flex; height:100%; flex-direction:column; justify-content:flex-end; padding:34px 40px; }.hero-context { margin:0 0 6px; color:#ffffffbf; font-size:.78rem; }.hero-title { max-width:75%; margin:0 0 6px; font-size:clamp(2.6rem,6vw,3.8rem); line-height:.95; }.hero-sub { margin:0; color:#ffffffdb; font-size:.92rem; }.hero-play { position:absolute; right:40px; bottom:36px; z-index:3; display:grid; place-items:center; width:56px; height:56px; border:0; border-radius:999px; color:#1b1008; background:var(--paper); cursor:pointer; transition:transform .4s cubic-bezier(.2,.8,.2,1); }.hero-play:hover { transform:scale(1.07); }.hero-play svg { width:18px; height:18px; fill:currentColor; }
  ...
  .collage { display:grid; grid-template-columns:repeat(6,1fr); grid-auto-rows:120px; gap:14px; }  .tile-wrap { position:relative; min-width:0; }
  .tile { position:relative; min-width:0; overflow:hidden; border:0; border-radius:26px; color:var(--paper); background:linear-gradient(140deg,var(--card-a,#3b2922),var(--card-b,#241a18)); cursor:pointer; transition:transform .5s cubic-bezier(.2,.8,.2,1),box-shadow .5s ease; }.tile:hover { transform:translateY(-4px); box-shadow:0 20px 38px #0008; }.tile.lead { grid-column:span 3; grid-row:span 3; }.tile.medium { grid-column:span 2; grid-row:span 2; text-align:left; }.tile.small { display:flex; grid-column:span 2; grid-row:span 1; align-items:center; gap:10px; padding:0 14px; text-align:left; }.tile-scrim { position:absolute; inset:0; z-index:1; background:linear-gradient(to top,#000b,transparent 60%); }.tile-caption { position:absolute; left:22px; right:22px; bottom:20px; z-index:2; display:flex; flex-direction:column; align-items:flex-start; }.tile-caption strong { font-size:1.9rem; line-height:.95; }.tile-caption small { margin-top:5px; color:#ffffffcc; font-size:.75rem; }.tile-title,.tile-artist { position:absolute; left:16px; z-index:2; overflow:hidden; max-width:calc(100% - 32px); text-overflow:ellipsis; white-space:nowrap; }.tile-title { bottom:27px; font-size:.82rem; }.tile-artist { bottom:10px; color:#ffffffbf; font-size:.68rem; }  .small-art { position:relative; display:grid; place-items:center; width:56px; height:56px; flex:none; overflow:hidden; border-radius:14px; background:#1c1512; }.small-art img { width:100%; height:100%; object-fit:cover; }.small-meta { min-width:0; }.small-meta strong,.small-meta small { display:block; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }.small-meta strong { font-size:.8rem; }.small-meta small { margin-top:3px; color:var(--muted); font-size:.7rem; }
  .row { display:grid; grid-template-columns:1.5fr 1fr 1fr 1fr; align-items:start; gap:14px; }.rowcard { position:relative; min-width:0; padding:14px; border:1px solid #ffffff0f; border-radius:22px; background:var(--card); transition:transform .4s cubic-bezier(.2,.8,.2,1),border-color .4s ease; }.rowcard:nth-child(2) { margin-top:26px; }.rowcard:nth-child(4) { margin-top:44px; }.rowcard:hover { border-color:#ffffff2b; transform:translateY(-3px); }.rowcard:nth-child(2):hover { transform:translateY(23px); }.rowcard:nth-child(4):hover { transform:translateY(41px); }.row-main { display:block; width:100%; min-width:0; padding:0; border:0; color:var(--paper); background:none; text-align:left; cursor:pointer; }  .art { position:relative; display:grid; place-items:center; width:100%; aspect-ratio:1; margin-bottom:10px; overflow:hidden; border-radius:16px; background:#1c1512; }.art::after { content:''; position:absolute; inset:0; z-index:2; background:linear-gradient(to top,#0008,transparent 55%); pointer-events:none; }.art img { width:100%; height:100%; object-fit:cover; }.row-main strong,.row-main small { display:block; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }.row-main strong { font-size:.8rem; }.row-main small { margin-top:3px; color:var(--muted); font-size:.72rem; }.source-badges { display:flex; gap:4px; margin-top:6px; }.source-badges em { padding:2px 5px; border:1px solid #ffffff18; border-radius:999px; color:var(--muted-2); background:#ffffff08; font-size:.57rem; font-style:normal; }.rowcard :global(.context-menu),.rowcard .add { position:absolute; right:9px; top:9px; }.add { width:28px; height:28px; border:0; border-radius:50%; color:var(--paper); background:#0008; cursor:pointer; }
  ...
  @media (max-width:800px) { .home-page { padding:20px 16px 140px; }.topbar { flex-wrap:wrap; }.wordmark { order:-1; width:100%; }.search { flex:1; width:auto; }.actions { margin-left:auto; }.btn { padding:9px 13px; }.collage { grid-template-columns:repeat(2,1fr); grid-auto-rows:110px; }.tile.lead { grid-column:span 2; grid-row:span 2; }.tile.medium { grid-column:span 1; grid-row:span 2; }.tile.small { grid-column:span 1; }.row,.playlist-row { grid-template-columns:repeat(2,1fr); }.rowcard:nth-child(2),.playlist-card:nth-child(2) { margin-top:18px; }.rowcard:nth-child(4),.playlist-card:nth-child(4) { margin-top:28px; }.playlist-feature { grid-column:span 2; }.smart-list { grid-template-columns:1fr; }.hero-title { max-width:85%; font-size:2.5rem; }.hero-content { padding:26px; }.hero-play { right:24px; bottom:24px; } }
  @media (max-width:520px) { .actions .ghost,.expired { display:none; }.settings { width:32px; }.pills { margin-bottom:24px; }.hero { height:240px; }.collage { gap:9px; grid-auto-rows:95px; }.row,.playlist-row { gap:9px; }.rowcard,.playlist-card { padding:10px; border-radius:18px; }.rowcard:nth-child(2),.playlist-card:nth-child(2) { margin-top:12px; }.rowcard:nth-child(4),.playlist-card:nth-child(4) { margin-top:20px; } }
  @media (prefers-reduced-motion: reduce) { .ambient-glow { animation:none; }.tile,.rowcard,.playlist-card,.btn,.hero-play { transition:none; } }
</style>
