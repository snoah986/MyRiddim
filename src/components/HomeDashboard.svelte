...
<section class="dashboard" aria-label="Home">
  <header class="dashboard-head"><div></div><div class="head-actions"><button on:click={onCreatePlaylist} aria-label="Create playlist">+ Playlist</button><button on:click={onOpenQueue} aria-label="Open queue">Queue <span>{upNext.length}</span></button></div></header>

  <section class="session-grid" aria-label="Live session monitor">
    <article class="session-deck">
      <div class="session-art">{#if active?.thumbnail}<img src={active.thumbnail} referrerpolicy="no-referrer" alt="{trackTitle(active)} artwork" />{:else}<span>♫</span>{/if}</div>
      <div class="session-copy"><h2>{trackTitle(active) === 'Untitled' && !active ? 'Nothing playing' : trackTitle(active)}</h2><p class="artist">{#if active}<button type="button" on:click|stopPropagation={() => onOpenArtist(active.artistId || active.artists?.[0]?.id || null, trackArtist(active))}>{trackArtist(active)}</button>{/if}</p>{#if active?.album}<button type="button" class="album" on:click|stopPropagation={() => onOpenAlbum(active.albumId || null, clean(active.album), trackArtist(active))}>{clean(active.album)}</button>{/if}<div class="progress"><span style={`width:${progress}%`}></span></div><div class="timecode"><span>{formatTime(currentTime)}</span><span>{formatTime(duration || durationValue(active))}</span></div></div>
